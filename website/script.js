...
function startDownload(url, btn) {
  if (!url) {
    setFeedback('File not found localy.');
    return;
  }
...
// Auto-detect device and handle links
if (isAndroid) {
  ...
  autoDownload.onclick = () => startDownload('./downloads/raizen-setup.exe', autoDownload.firstElementChild);
}
...
document.getElementById('check-updates').onclick = () => {
  ...
  setTimeout(() => {
    setFeedback('You have the latest version.');
  }, 1000);
};
